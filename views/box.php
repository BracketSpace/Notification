...
<div id="<?php $this->echo_var( 'id' ); ?>" class="postbox <?php echo ( ! $this->get_var( 'open' ) && $this->get_var( 'active' ) ) ? 'closed' : ''; ?>" data-nt-carrier <?php echo ( ! $this->get_var( 'active' ) ) ? 'data-nt-hidden' : ''; ?>>
	<div class="switch-container">
		<label class="switch <?php echo ( $this->get_var( 'open' ) || ! $this->get_var( 'active' ) ) ? 'active' : ''; ?>">
			<input type="checkbox" name="<?php $this->echo_var( 'name' ); ?>" value="1" <?php checked( ( $this->get_var( 'open' ) || ! $this->get_var( 'active' ) ), true ); ?> />
			<div></div>
		</label>
