...
class TermAdded extends TermTrigger {
	...
	public function __construct( $taxonomy = 'category' ) {
		...
		parent::__construct( [
			...
			'name'     => sprintf( __( '%s term created', 'notification' ), $taxonomy_name ),
		] );

		$this->add_action( 'created_' . $taxonomy, 100, 2 );

		// translators: 1. taxonomy name, 2. taxonomy slug.
		$this->set_description( sprintf( __( 'Fires when %1$s (%2$s) is created', 'notification' ), $taxonomy_name, $taxonomy ) );
