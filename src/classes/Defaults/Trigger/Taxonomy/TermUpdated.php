...
class TermUpdated extends TermTrigger {
	...
	public function __construct( $taxonomy = 'category' ) {
		...
		$this->set_description( sprintf( __( 'Fires when %1$s (%2$s) is updated', 'notification' ), parent::get_taxonomy_name( $taxonomy ), $taxonomy ) );
	...
	public function action( $term_id ) {
		...
		$this->term_modification_datetime = $this->cache( 'update_timestamp', current_time( 'timestamp' ) ); // phpcs:ignore
