	...
	public function get_users_by_role( $role ) {
		...
		if ( empty( $users ) ) {
			$users = $wpdb->get_results( //phpcs:ignore
				$wpdb->prepare( "SELECT u.ID, u.user_email, u.display_name FROM $wpdb->users AS u INNER JOIN $wpdb->usermeta AS m ON u.ID = m.user_id WHERE m.meta_key = '{$wpdb->get_blog_prefix()}_capabilities' AND m.meta_value LIKE %s", '%' . $wpdb->esc_like( $role ) . '%' )
			);
			$cache->set( $users );
		}
