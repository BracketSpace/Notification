...
class TaxonomyAdded extends TaxonomyTrigger {
	...
	public function __construct( $post_type = 'post' ) {
		...
		parent::__construct( array(
			...
			'name'      => sprintf( __( '%s added', 'notification' ), parent::get_taxonomy_name( $post_type ) ),
		) );

		$this->add_action( 'wp_insert_post', 10, 3 );

		// translators: 1. singular post name, 2. post type slug.
		$this->set_description( sprintf( __( 'Fires when %s (%s) is added to database. Useful when adding posts programatically or for 3rd party integration', 'notification' ), parent::get_post_type_name( $post_type ), $post_type ) );
