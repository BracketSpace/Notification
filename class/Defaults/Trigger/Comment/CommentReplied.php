...
class CommentReplied extends CommentTrigger {
	...
	public function merge_tags() {
		...
		// Parent comment.
		$this->add_merge_tag( new MergeTag\Comment\CommentID( array(
			'slug'          => 'parent_comment_ID',
			'name'          => __( 'Parent comment ID', 'notification' ),
			'property_name' => 'parent_comment',
		) ) );

		$this->add_merge_tag( new MergeTag\Comment\CommentContent( array(
			'slug'          => 'parent_comment_content',
			'name'          => __( 'Parent comment content', 'notification' ),
			'property_name' => 'parent_comment',
		) ) );

		$this->add_merge_tag( new MergeTag\Comment\CommentStatus( array(
			'slug'          => 'parent_comment_status',
			'name'          => __( 'Parent comment status', 'notification' ),
			'property_name' => 'parent_comment',
		) ) );

		$this->add_merge_tag( new MergeTag\Comment\CommentType( array(
		...
		$this->add_merge_tag( new MergeTag\Comment\CommentAuthorIP( array(
			'slug'          => 'parent_comment_author_IP',
			'name'          => __( 'Parent comment author IP', 'notification' ),
			'property_name' => 'parent_comment',
		) ) );

		$this->add_merge_tag( new MergeTag\Comment\CommentAuthorUserAgent( array(
			'slug'          => 'parent_comment_user_agent',
			'name'          => __( 'Parent comment user agent', 'notification' ),
			'property_name' => 'parent_comment',
		) ) );

		$this->add_merge_tag( new MergeTag\Comment\CommentAuthorUrl( array(
